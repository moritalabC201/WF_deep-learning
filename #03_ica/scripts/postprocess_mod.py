...
def parse_areanames(matfile_path:str):
    '''
    INPUT
    matfile_path is file path. The .mat file include info about pair of areanames and areaindex_number 
    info["areainds"] is python list of integer, which indicates single atlas region of CCF. [1,2,...25, -1,-2,...-25]
    info["areanames"] is python list of str, which is in the form of "acronym" + "_" + "R/L"
    '''    
    import scipy.io as sio
    info = sio.loadmat(matfile_path, squeeze_me=True)
    areainds = info["areainds"]
    areanames = info["areanames"]
    sortvec=np.argsort(areainds)
    areanames=[areanames[i] for i in sortvec]
    areainds=[areainds[i] for i in sortvec]
    return areainds,areanames

def plot_components(A,C,areas,atlas,matfile_path,outputfolder):
   # Make figures directory
    figdir=os.path.join(outputfolder,'Figures')
    if not os.path.exists(figdir):
        os.makedirs(figdir)
    
    areainds,areanames = parse_areanames(matfile_path)
...
def plot_correlations(A,C,areas,atlas,matfile_path,outputfolder):
    # Correlation plot
   # Make figures directory
    figdir=os.path.join(outputfolder,'Figures')
    if not os.path.exists(figdir):
