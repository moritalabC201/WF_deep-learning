...
def jadeR(X, m=None, verbose=False):
    ...
    # 2. 成分時系列（ホワイトニング＋回転済み）
    Y = B @ Xc

    # 3. 混合行列の推定値
    A_hat = np.linalg.pinv(B)   # shape = (n_sensors, m)

    # 4. 各成分 i の「元スケールでのエネルギー」（分散に相当）
    energies = np.sum(A_hat**2, axis=0)   # shape = (m,)

    #for i, v in enumerate(variances):
    #    print(f"Component {i+1} variance: {v:.4f}")

    return B.astype(origtype), energies